...
import {mapDidacticUnitToSetupViewModel} from "@/dashboard/viewModelMappers";
import type {UnitSetupViewModel, PlanningSyllabus} from "@/dashboard/types";
import {TopicStep} from "./steps/TopicStep";
import {QuestionnaireStep} from "./steps/QuestionnaireStep";
import {SyllabusStep} from "./steps/SyllabusStep";
import {getUnitGenerationCost} from "@/dashboard/utils/generationCosts";
import {
...
export function CreateUnitWizard({
	...
	const [planning, setPlanning] = useState<UnitSetupViewModel | null>(
		null,
	);
	const [activeUnitId, setActiveUnitId] = useState<string | null>(
		didacticUnitId ?? null,
	);
	const [availableFolders, setAvailableFolders] = useState<FolderDto[]>(
		[],
	);
	...
	const applyPlanningState = useCallback(
		(detail: Awaited<ReturnType<typeof dashboardApi.getDidacticUnit>>) => {
			const pd = mapDidacticUnitToSetupViewModel(detail);
			setPlanning(pd);
	...
	const handleTopicSubmit = useCallback(async () => {
		...
		try {
			...
			onDataChanged();
			const pd = mapDidacticUnitToSetupViewModel(created);
			setPlanning(pd);
			setQuestionnaireAnswers(pd.questionnaire?.answers ?? {});
			setActiveUnitId(pd.id);
	...
	const handleGenerateSyllabus = useCallback(
		async (tier: GenerationQualityDto) => {
			if (!planning) return;
			setIsSubmitting(true);
		...
		[planning, refreshUser, reviewDecision, regenerationContext, onDataChanged],
	);
	...
	const handleStartGeneration = useCallback(
		...
		[planning, refreshUser, onDataChanged, onOpenEditor],
	);
