...
function activityTypeLabel(type: LearningActivityDto["type"]): string {
	switch (type) {
...
function activityTypeDescription(type: LearningActivityDto["type"]): string {
	switch (type) {
		case "multiple_choice":
...
function ActivityIcon({type}: {type: LearningActivityDto["type"]}) {
	const Icon =
...
function isDraftPersistedActivity(type: LearningActivityDto["type"]) {
	return (
		type === "case_study" ||
		type === "guided_project" ||
...
function ShortAnswerActivity({
	...
	latestAttempt?: LearningActivityAttemptDto;
...
function CodePracticeActivity({
	...
	useEffect(() => {
		if (latestAttemptId) {
			// Keep the feedback tab synced when a new generated attempt arrives.
...
function VirtualFileDialog({
	...
	useEffect(() => {
		// Reset the editable draft whenever the dialog is reused for another file.
		// eslint-disable-next-line react-hooks/set-state-in-effect
...
function ActivityFeedbackPanel({
	...
	attempt: LearningActivityAttemptDto;
	className?: string;
}) {
...
function FlashcardsActivity({activity}: {activity: LearningActivityDto}) {
	const cards = useMemo(
		() =>
...
export function LearningActivityRenderer({
	...
	activity: LearningActivityDto;
	attempts: LearningActivityAttemptDto[];
	contentScale?: number;
