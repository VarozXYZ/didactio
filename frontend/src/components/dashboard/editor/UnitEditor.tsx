...
import {
	type DidacticUnitReadingProgressResponseDto,
	type GenerationQualityDto,
	type DidacticUnitNoteDto,
	type DidacticUnitNoteAnchorDto,
	type DidacticUnitChapterDetailDto,
	type GenerationRunDto,
	type LearningActivityDto,
	type LearningActivityAttemptDto,
	type LearningActivityScopeDto,
...
import {
	mapDidacticUnitToEditorViewModel,
	mapDidacticUnitRevisionsToViewModels,
} from "@/dashboard/viewModelMappers";
...
import {formatRelativeTimestamp} from "@/dashboard/utils/unitDisplayMetadata";
import {
...
import {
	...
	themeVars,
} from "@/shared/presentation/themeVars";
import {
...
function activityTypeOutlineLabel(type: LearningActivityTypeDto): string {
	switch (type) {
...
function sourceLabel(source: UnitRevisionViewModel["source"]): string {
	switch (source) {
		case "ai_generation":
...
function formatRunLabel(run: GenerationRunDto): string {
...
type MobileUnitEditorProps = {
	workspace: UnitEditorViewModel;
	activeChapter: UnitEditorChapterViewModel;
	activeChapterIndex: number;
	activeContentHtml: string;
	learningActivitiesByChapter: Record<number, LearningActivityDto[]>;
	activityAttempts: Record<string, LearningActivityAttemptDto[]>;
...
export function UnitEditor({didacticUnitId, onDataChanged}: UnitEditorProps) {
	...
	const [workspace, setWorkspace] =
		useState<UnitEditorViewModel | null>(null);
	const [chapterDetails, setChapterDetails] = useState<
		Record<number, DidacticUnitChapterDetailDto>
	>({});
	const [revisions, setRevisions] = useState<UnitRevisionViewModel[]>(
		[],
	);
	...
	const [learningActivities, setLearningActivities] = useState<
		Record<number, LearningActivityDto[]>
	>({});
	const [activityAttempts, setActivityAttempts] = useState<
		Record<string, LearningActivityAttemptDto[]>
	>({});
	...
	const [activityType, setActivityType] =
		useState<LearningActivityTypeDto>("multiple_choice");
	const [activityQuality, setActivityQuality] =
		useState<GenerationQualityDto>("silver");
	...
	const [unitGenerationTier, setUnitGenerationTier] =
		useState<GenerationQualityDto | null>(null);
	...
	const displayTextStyle = useMemo(
		() =>
			...
			:	undefined,
		[draft, shouldUseCompactDesktopTextSizeDefault],
	);
	useEffect(() => {
	...
	const hasNextActiveModule = useMemo(
		() =>
			...
			:	false,
		[activeChapter, workspace?.chapters.length],
	);
	const activeRuns = [] as GenerationRunDto[];

	const activeLearningActivities = useMemo(
		() =>
			activeChapter ?
	...
	const loadRevisions = useCallback(
		async (chapterIndex: number) => {
			try {
				...
				setRevisions(mapDidacticUnitRevisionsToViewModels(response.revisions));
			} catch (loadError) {
				if (
	...
	const loadWorkspace = useCallback(
		async (
			...
			try {
				...
				) as Record<number, DidacticUnitChapterDetailDto>;
				const detailMap = new Map(
					detailResponses.map(
						(detail) => [detail.chapterIndex, detail] as const,
					),
				);
				const nextWorkspace = mapDidacticUnitToEditorViewModel({
					unit,
	...
	const startEditingNote = useCallback((note: DidacticUnitNoteDto) => {
		setEditingNoteId(note.id);
	...
	const reconcileReadingProgress = useCallback(
		(response: DidacticUnitReadingProgressResponseDto) => {
			if (!response.module) {
				return;
			}
	...
	const handlePrimaryGeneration = async (
		tierOverride?: GenerationQualityDto,
	) => {
	...
	const isRevisionCurrent = (revision: UnitRevisionViewModel) => {
	...
	const handleRestoreRevision = async (
		revision: UnitRevisionViewModel,
	) => {
		if (!activeChapter) {
			return;
		}
	...
	const getStatusIcon = (chapter: UnitEditorChapterViewModel) => {
		const isGenerating =
			isStreamingGeneration &&
			activeGeneratingChapterIndex !== null &&
	...
	const handleDeleteLearningActivity = async (activityId: string) => {
		try {
			...
			setLearningActivities((previous) => {
				const next: Record<number, LearningActivityDto[]> = {};
