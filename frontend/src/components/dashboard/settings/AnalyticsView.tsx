...
function ProviderIcon({
	...
	const logo = getProviderLogo(provider, resolvedMode);
...
function FavoriteTopicHighlight({analytics}: {analytics: UsageAnalyticsDto}) {
...
function ActivityChart({
	...
	onPeriodChange: (period: UsageAnalyticsPeriodDto) => void;
	dark: boolean;
}) {
	return (
...
export function AnalyticsView() {
	const {resolvedMode} = useAppearance();
	const [period, setPeriod] = useState<UsageAnalyticsPeriodDto>("30d");
	const [analytics, setAnalytics] = useState<UsageAnalyticsDto | null>(null);
	const [isLoading, setIsLoading] = useState(true);
	const [error, setError] = useState<string | null>(null);

	useEffect(() => {
		let isMounted = true;
		// Mark this fetch cycle as pending before subscribing to the async result.
