...
import {getProviderLogo as getLogoForProvider} from "@/shared/models/generationModels";
import {
	type AiConfigDto,
	type AiModelConfigDto,
	type AuthoringConfigDto,
...
function modelIdFromConfig(config: AiModelConfigDto) {
	return `${config.provider}/${config.model}`;
}

function modelConfigFromId(id: string): AiModelConfigDto {
...
function getProviderLogo(modelId: string, dark: boolean): string | undefined {
	...
	return getLogoForProvider(provider, dark ? "dark" : "light");
}

function normalizeAiConfig(config: AiConfigDto): AiConfigDto {
...
function ModelColumn({
	...
	catalog: ModelEntryDto[];
	current: AiModelConfigDto;
	tier: ModelTier;
	onChange: (next: AiModelConfigDto) => void;
}) {
