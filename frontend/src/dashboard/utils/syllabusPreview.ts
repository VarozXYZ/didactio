...
function parseChapters(block: string): PartialPlanningSyllabusChapter[] {
    return splitSections(block, 3).map((chapterSection) => {
        ...
        const overviewSection = innerSections.find((entry) => /overview/i.test(entry.heading))
        ...
        return {
            ...
            overview: overviewSection ? parseParagraph(overviewSection.body) : undefined,
            keyPoints: keyPointsSection ? parseBulletList(keyPointsSection.body) : [],
            estimatedDurationMinutes: estimatedDurationSection
...
export function parsePartialSyllabusMarkdown(markdown: string): PartialPlanningSyllabus | null {
    ...
    return {
        ...
        overview: overviewSection ? parseParagraph(overviewSection.body) : undefined,
        learningGoals: learningGoalsSection ? parseBulletList(learningGoalsSection.body) : [],
