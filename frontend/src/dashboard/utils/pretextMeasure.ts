...
// ─── Block constructors for each semantic role ────────────────────────────────

/**
 * Prepare a body paragraph block.
 * `inlineChildren` is accepted for API compatibility but Pretext v0.0.5 measures
 * in a single font pass; the plain-text representation is used.
 */
...
export function prepareH1Block(
    ...
    return makeBlock(plainText, h.fontString, h.sizePx * h.lineHeight, h.marginTopPx, h.marginBottomPx)
}
...
/** Return the total height in px a block will occupy at the given content width. */
export function measureBlockHeight(measurement: BlockMeasurement, contentWidth: number): number {
    if (contentWidth <= 0) return 0
    const { lineCount } = measureLineStats(measurement.prepared, contentWidth)
    return measurement.marginTopPx + lineCount * measurement.lineHeightPx + measurement.marginBottomPx
}

/** Return the number of visual lines at the given content width. */
export function blockLineCount(measurement: BlockMeasurement, contentWidth: number): number {
    return measureLineStats(measurement.prepared, contentWidth).lineCount
}

// ─── Line-based splitting ─────────────────────────────────────────────────────

/** Return the plain text of each visual line at the given content width. */
export function getBlockLines(measurement: BlockMeasurement, contentWidth: number): string[] {
    const lines: string[] = []
...
/**
 * Split a block at a line boundary.
 * Returns the plain text for the fitting portion and the remainder.
 */
