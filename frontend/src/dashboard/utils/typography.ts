...
export const FONT_CATALOG = {
    inter:        { label: 'Inter',        family: 'Inter',            category: 'sans-serif',  googleId: null /* already in index.css */ },
    roboto:       { label: 'Roboto',       family: 'Roboto',           category: 'sans-serif',  googleId: 'Roboto:wght@400;500;700' },
    montserrat:   { label: 'Montserrat',   family: 'Montserrat',       category: 'sans-serif',  googleId: 'Montserrat:wght@400;600;700' },
    nunito:       { label: 'Nunito',       family: 'Nunito',           category: 'sans-serif',  googleId: 'Nunito:wght@400;600;700' },
    openSans:     { label: 'Open Sans',    family: 'Open Sans',        category: 'sans-serif',  googleId: 'Open+Sans:wght@400;600;700' },
    merriweather: { label: 'Merriweather', family: 'Merriweather',     category: 'serif',       googleId: 'Merriweather:wght@400;700' },
    ebGaramond:   { label: 'Garamond',     family: 'EB Garamond',      category: 'serif',       googleId: 'EB+Garamond:wght@400;600;700' },
    playfair:     { label: 'Playfair',     family: 'Playfair Display', category: 'serif',       googleId: 'Playfair+Display:wght@400;700' },
    lora:         { label: 'Lora',         family: 'Lora',             category: 'serif',       googleId: 'Lora:wght@400;600;700' },
    playwrite:    { label: 'Playwrite IE', family: 'Playwrite IE',     category: 'handwriting', googleId: 'Playwrite+IE:wght@400' },
} as const
...
// Pixel sizes per profile per element, for both mobile and desktop viewports.
// "regular" matches the sizes previously hardcoded in index.css and pretextMeasure.ts.
const SIZE_PROFILES: Record<SizeProfile, {
...
// em-based constants shared across all profiles (defined in index.css).
const LINE_HEIGHT_BODY      = 1.9
const LINE_HEIGHT_HEADING   = 1.25  // leading-tight
const MARGIN_BOTTOM_BODY_EM = 0.8   // .prose p
const MARGIN_TOP_H2_EM      = 1.2   // .prose h2
const MARGIN_BOTTOM_H2_EM   = 0.5
const MARGIN_TOP_H3_EM      = 1.0   // .prose h3
const MARGIN_BOTTOM_H3_EM   = 0.4
const LIST_MARGIN_TOP_EM    = 0.5   // .prose ul, ol
const LIST_MARGIN_BOTTOM_EM = 0.8
const LIST_PADDING_LEFT_EM  = 1.2
const LIST_ITEM_MARGIN_EM   = 0.3   // .prose li
const CODE_FONT_SIZE_EM     = 0.92  // .prose code
const CODE_PADDING_H_EM     = 0.35  // .prose code padding-left/right
const BLOCKQUOTE_INDENT_EM  = 1.0   // pl-4 equivalent
const BLOCKQUOTE_BORDER_PX  = 4     // border-l-4

function cssFont(weight: number, sizePx: number, family: string, italic = false): string {
...
export type ResolvedTypography = {
    ...
    h1: { family: string; sizePx: number; lineHeight: number; marginTopPx: number; marginBottomPx: number; fontString: string }
    h2: { family: string; sizePx: number; lineHeight: number; marginTopPx: number; marginBottomPx: number; fontString: string; boldFontString: string }
    h3: { family: string; sizePx: number; lineHeight: number; marginTopPx: number; marginBottomPx: number; fontString: string; boldFontString: string }
...
// CSS custom property names used in index.css and set on measurement / editor containers.
...
// Returns a React-compatible style object of CSS custom properties to spread on a container.
export function makeTypographyVars(resolved: ResolvedTypography): Record<string, string> {
    return {
...
// Apply typography CSS vars directly to a DOM element (for the measurement sandbox).
export function applyTypographyVars(element: HTMLElement, resolved: ResolvedTypography): void {
    ...
    // Also set font-family directly so the element (and all children without explicit font-family) inherit it
