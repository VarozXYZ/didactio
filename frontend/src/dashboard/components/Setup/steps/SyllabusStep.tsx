    ...
    return (
        <>
            ...
            <button
                ...
                onClick={onClick}
                disabled={disabled}
                onMouseEnter={() => { if (!disabled) { setHovered(true); setSpinKey(k => k + 1) } }}
                ...
                style={{
                    ...
                    boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.10), inset 0 -2px 8px rgba(0,0,0,0.30)',
                }}
            >
                <div
                    aria-hidden="true"
                    style={{
                        ...
                        boxShadow: '0 20px 52px -22px rgba(0,0,0,0.78), 0 12px 26px -18px rgba(0,0,0,0.58)',
                    }}
                />
                {hovered && (
                    <div
                        ...
                        style={{
                            position: 'absolute', width: '300%', height: '500%', top: '50%', left: '50%',
                            ...
                            WebkitMaskImage: 'conic-gradient(from 0deg, black 0deg, black var(--arc-end-sg), transparent calc(var(--arc-end-sg) + 12deg), transparent 360deg)',
                            maskImage: 'conic-gradient(from 0deg, black 0deg, black var(--arc-end-sg), transparent calc(var(--arc-end-sg) + 12deg), transparent 360deg)',
                            zIndex: 1,
                        }}
                    />
                )}
...
function ChapterAccordion({ chapter, index, isOpen, onHover, onLeave }: {
    ...
    return (
        <div
            ...
            style={{ background: 'rgba(255,255,255,0.55)', border: '1px solid rgba(0,0,0,0.06)', backdropFilter: 'blur(12px)' }}
        ...
        >
            ...
            <div
                ...
                style={{ height, transitionTimingFunction: 'cubic-bezier(0.4, 0, 0.2, 1)' }}
            >
                    ...
                    {lessons.length > 0 && (
                        <div className="space-y-3 pl-9">
                            {lessons.map((lesson, li) => (
                                <div key={`${lesson.title}-${li}`} className="flex items-start gap-2.5">
                                        ...
                                        {(lesson.contentOutline?.length ?? 0) > 0 && (
                                            <ul className="mt-0.5 space-y-0.5">
                                                {lesson.contentOutline?.map((o) => (
                                                    <li key={o} className="flex gap-1.5 text-[11px] leading-[1.55] text-[#86868B]">
                                                        <span className="mt-[1px] shrink-0 text-[#D1D1D6]">–</span>
                                                        {o}
                                                    </li>
                                                ))}
                                            </ul>
...
function SyllabusCard({ syllabus, isStreaming }: {
    ...
    return (
        <div className="space-y-5">
            ...
            {(syllabus.overview || (syllabus.keywords?.length ?? 0) > 0) && (
                <div className="grid gap-x-5 gap-y-3" style={{ gridTemplateColumns: '1fr auto', alignItems: 'start' }}>
                    {syllabus.overview && (
                        <div>
                            <div className="mb-1 text-[10px] font-bold uppercase tracking-widest text-[#AEAEB2]">Overview</div>
                            <p className="text-[13px] leading-[1.75] text-[#6E6E73]">{syllabus.overview}</p>
                        </div>
                    )}

                    {(syllabus.keywords?.length ?? 0) > 0 && (
                        <div className="w-[200px] shrink-0">
                            <div className="mb-1.5 text-[10px] font-bold uppercase tracking-widest text-[#AEAEB2]">Keywords</div>
                            <div className="flex flex-wrap gap-1.5">
                                {syllabus.keywords
                                    ...
                                    .map((kw) => (
                                        <span key={kw} className="rounded-full bg-[#EAF9EF] px-2.5 py-[3px] text-[11px] font-medium text-[#2D8F4B]">
                                            {kw}
                                        </span>
                                    ))}
                            </div>
...
export function SyllabusStep({
    ...
    return (
        <div className="space-y-4">
            ...
            {hasSyllabus && (
                <div className="flex flex-col items-center gap-5 border-t pt-6" style={{ borderColor: 'rgba(0,0,0,0.06)' }}>
                    {/* Question label */}
                    <div className="flex items-center gap-2 rounded-full px-4 py-1.5" style={{ background: 'rgba(52,199,89,0.08)', border: '1px solid rgba(52,199,89,0.2)' }}>
                        <span className="text-[13px]">🎉</span>
                        <p className="text-[13px] font-semibold text-[#1D1D1F]">Happy with this syllabus?</p>
                    </div>
                    ...
                    {!hasChosen ? (
                        ...
                        <div className="flex animate-in fade-in items-center gap-2.5 duration-200">
                            <span className="rounded-[10px] px-4 py-1.5 text-[12.5px] font-semibold" style={
                                reviewDecision === 'accept'
                                    ? { background: 'rgba(52,199,89,0.10)', border: '1px solid rgba(52,199,89,0.22)', color: '#2D8F4B' }
                                    : { background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', color: '#1D1D1F' }
                            }>
                                {reviewDecision === 'accept' ? '😊 Yes, looks great' : '😕 No, tweak it'}
                            </span>
                            <button
                                type="button"
                                onClick={undoChoice}
                                ...
                                title="Change answer"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 64 640 512" width="13" height="13" fill="currentColor">
                    ...
                    {hasChosen && reviewDecision === 'accept' && (
                        <div className="flex animate-in fade-in slide-in-from-bottom-2 flex-col items-center gap-3 duration-200">
                            <div className="inline-flex rounded-[10px] bg-[#F5F5F7] p-0.5" style={{ border: '1px solid rgba(0,0,0,0.06)' }}>
                                <button
                                    type="button"
                            ...
                            <StartGenerationButton
                                ...
                                label={isSubmitting ? 'Starting…' : 'Start learning'}
                            />
                        </div>
                    )}
                            ...
                            <div className="flex items-stretch gap-3">
                                ...
                                <div className="flex shrink-0 flex-col items-stretch gap-2">
                                    ...
                                    <button
                                        ...
                                        onClick={() => void onGenerateSyllabus(selectedGenerationTier)}
                                    ...
                                    >
                                        {isStreamingSyllabus ? 'Regenerating…' : 'Regenerate'}
                                    </button>
                                </div>
                            </div>
                        </div>
