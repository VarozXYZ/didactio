		...
		return editor.registerUpdateListener(
			({editorState, dirtyElements, dirtyLeaves}) => {
				...
				editorState.read(() => {
					...
					onMarkdownChange(
						normalizeMarkdownForStorage(htmlToStoredMarkdown(html)),
					);
				});
			},
		);
...
export function LexicalMarkdownEditor({
	...
	useEffect(() => {
		...
		if (baseTextStyle?.bodyFontFamily)
			fontsToLoad.push(baseTextStyle.bodyFontFamily as FontId);
		if (baseTextStyle?.headingFontFamily)
			fontsToLoad.push(baseTextStyle.headingFontFamily as FontId);
	...
	const contentStyle = {
		...
		textAlign: (baseTextStyle?.paragraphAlign ?? "left") as
			| "left"
			| "center"
			| "right"
			| "justify",
