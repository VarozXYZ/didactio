...
describe("prompt quality helpers", () => {
	it("builds a moderation prompt with the improved brief contract", () => {
		const prompt = buildModerationPrompt({
			...
			level: "beginner",
			additionalContext:
				"Focus on beginners building small useful scripts.",
			folders: [
				{
					name: "Computer Science",
					description:
						"Use for units primarily focused on computer science.",
				},
				{
					name: "General",
					description:
	...
	it("builds a chapter prompt with continuity, module planning, and regeneration guidance", () => {
		const prompt = buildChapterMarkdownPrompt({
			...
			instruction:
				"Use more concrete examples than the previous version.",
			authoring,
			depth: "technical",
			length: "textbook",
			syllabus: {
	...
	it("falls back to not provided when questionnaire answers are skipped", () => {
		...
		const chapterPrompt = buildChapterMarkdownPrompt({
			...
			syllabus: {
				...
				description:
					"An introduction to trade routes and cultural exchange.",
	...
	it("adapts the reference syllabus schema into the compatibility syllabus object", () => {
		...
		const syllabus =
			adaptReferenceSyllabusToDidacticUnitSyllabus(referenceSyllabus);

		expect(buildGatewaySystemPrompt("syllabus")).toContain(
			"curriculum designer",
		);
		expect(syllabus.keywords).toEqual([
			"python",
			"scripting",
			"automation",
		]);
