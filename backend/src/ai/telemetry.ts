...
function normalizeUsage(usage: UsageLike | undefined): AiUsageTelemetry | undefined {
    if (!usage) {
        return undefined
    }
...
function normalizeRequest(request: RequestLike | undefined): AiRequestTelemetry | undefined {
    if (!request) {
        return undefined
    }
...
function normalizeResponse(response: ResponseLike | undefined): AiResponseTelemetry | undefined {
    if (!response) {
        return undefined
    }
...
function extractGatewayGenerationId(providerMetadata: unknown): string | undefined {
    ...
    const generationId = (gatewayValue as { generationId?: unknown }).generationId
    return typeof generationId === 'string' && generationId.trim() ? generationId : undefined
}
...
export async function collectAiCallTelemetry(
    ...
    return compactRecord({
        ...
        warnings: warnings ? (toSerializableValue(warnings) as unknown[]) : undefined,
        request: normalizeRequest(request),
        response: normalizeResponse(response),
        providerMetadata: serializedProviderMetadata,
        gatewayGenerationId: extractGatewayGenerationId(serializedProviderMetadata),
    })
}
...
export function summarizeAiCallTelemetry(
    ...
    return compactRecord({
        ...
        inputTokens: telemetry.totalUsage?.inputTokens ?? telemetry.usage?.inputTokens,
        outputTokens: telemetry.totalUsage?.outputTokens ?? telemetry.usage?.outputTokens,
        totalTokens: telemetry.totalUsage?.totalTokens ?? telemetry.usage?.totalTokens,
...
function createGatewayUsageTelemetry(
    ...
    const merged = compactRecord({
        ...
        cachedInputTokens: usage?.cachedInputTokens ?? gateway.cachedInputTokens,
    })

    return Object.keys(merged).length > 0 ? (merged as AiUsageTelemetry) : undefined
}
...
export function enrichAiCallTelemetryWithGatewayInfo(
    ...
    const totalUsage = createGatewayUsageTelemetry(telemetry.totalUsage, gateway)
