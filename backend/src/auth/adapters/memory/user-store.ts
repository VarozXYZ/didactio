...
export class InMemoryUserStore implements UserStore {
	...
	async grantLaunchCredits(
		...
		const updated: AuthUser = {
			...user,
			credits: {
				bronze: (currentCredits.bronze ?? 0) + credits.bronze,
				silver: (currentCredits.silver ?? 0) + credits.silver,
				gold: (currentCredits.gold ?? 0) + credits.gold,
				dark: (currentCredits.dark ?? 0) + credits.dark,
			},
	...
	async applyCreditDelta(input: {
		...
		const nextValue = (credits[input.coinType] ?? 0) + input.delta;
