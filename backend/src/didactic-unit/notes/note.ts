...
function parseAnchor(value: unknown): DidacticUnitNoteAnchor {
  ...
  const startOffset = parseNonNegativeInteger(
    payload.startOffset,
    "anchor.startOffset",
  );
  const endOffset = parseNonNegativeInteger(
    payload.endOffset,
    "anchor.endOffset",
  );
  if (payload.startBlockId === payload.endBlockId && endOffset <= startOffset) {
    throw new Error(
      "anchor.endOffset must be greater than anchor.startOffset.",
    );
  }

  return {
    startBlockId: parseNonEmptyString(
      payload.startBlockId,
      "anchor.startBlockId",
    ),
    startOffset,
