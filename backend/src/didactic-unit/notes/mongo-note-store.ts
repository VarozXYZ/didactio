...
import type { DidacticUnitNote } from "./note.js";
import type { DidacticUnitNoteStore } from "./note-store.js";

type DidacticUnitNoteDocument = DidacticUnitNote & Document;

function stripMongoId(
  document: DidacticUnitNoteDocument | null,
): DidacticUnitNote | null {
  if (!document) {
    return null;
  }
