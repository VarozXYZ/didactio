...
export function formatModuleMarkdownForReadProgress(markdown: string | null | undefined): string {
...
export function getModuleReadCharacterCount(
    ...
    const totalCharacterCount = getModuleTotalCharacterCount(didacticUnit, moduleIndex)
    ...
    return Math.min(
        getModuleReadProgressRecord(didacticUnit, moduleIndex)?.readCharacterCount ?? 0,
        totalCharacterCount
    )
}
...
export function updateDidacticUnitModuleReadProgress(
    ...
    const existingProgress = getModuleReadProgressRecord(didacticUnit, moduleIndex)
...
export function resetDidacticUnitModuleReadProgress(
    ...
    const nextModuleReadProgress = (didacticUnit.moduleReadProgress ?? []).filter(
        (progress) => progress.moduleIndex !== moduleIndex
    )

    if (nextModuleReadProgress.length === (didacticUnit.moduleReadProgress ?? []).length) {
        return didacticUnit
    }
